...
document.getElementById('all').addEventListener('click', function() {
    ...
    tr = table.querySelector('tbody').getElementsByTagName('tr');
    for (var i = 0; i < tr.length; i++) {
        // Remove the 'donotShow_row' class
        tr[i].classList.remove('donotShow_row');
    }
});
...
document.getElementById('completed').addEventListener('click', function () {
    ...
    // Function to hide the filter dropdown on small screens
    ...
    // Add an event listener to check the screen width and hide the dropdown on load and resize
    window.addEventListener('load', hideFilterDropdown);
    window.addEventListener('resize', hideFilterDropdown);
